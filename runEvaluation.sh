...
-XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:NewSize=1024m -XX:SurvivorRatio=16
...
function executeAnyBenchmark() {
	...
	#echo "$DIR_A"
	local DIR_A2=$RESULT_DIR/$benchmark_name"_A2"$vm_memory_argsA
	#echo "$DIR_A2"
	local DIR_A3=$RESULT_DIR/$benchmark_name"_A3"$vm_memory_argsA
	#echo "$DIR_A3"	
	local DIR_B=$RESULT_DIR/$benchmark_name"_B"$vm_memory_argsB
	#echo "$DIR_B"
	local DIR_AA=$RESULT_DIR/$benchmark_name"_AA"$vm_memory_argsA
	#echo "$DIR_AA"
	local DIR_BB=$RESULT_DIR/$benchmark_name"_BB"$vm_memory_argsB
	#echo "$DIR_BB"
	...
	if [[ "$benchmakr_run_nr" == "--" ]]; then 
		...
		local DIR_AB=$RESULT_DIR/"_"$benchmark_name$vm_memory_argsA$vm_memory_argsB/$benchmakr_run_nr
		#echo "$DIR_AB"		
	fi;
...
executeRascalShellBenchmark "MT20" "MOD17_EVALTREE_15"
#
